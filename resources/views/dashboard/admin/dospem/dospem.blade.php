...
@section('content')
    <div class="card-body shadow-[0px_5px_60px_-15px_rgba(0,0,0,0.4)] px-3 py-5 rounded-md">
        <div class="flex mb-3 justify-center">
            <h1 class="font-bold text-xl">DATA DOSEN PEMBIMBING UNIVERSITAS</h1>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($dosen as $user)
                            <tr>
                                ...
                                <td class="px-2 py-4 border border-gray-400">{{ $user->nidn }}</td>
                                <td class="px-2 py-4 border border-gray-400">{{ $user->name }}</td>
                                <td class="px-2 py-4 border border-gray-400">{{ $user->email }}</td>
                                <td class="px-2 py-4 border border-gray-400">{{ $user->jabatan }}</td>
                                <td class="px-2 py-4 border border-gray-400">
                                    <a href="{{ route('admin.data_dospem.info', ['id' => $user->id]) }}"><button
                                            class="px-4 py-2 bg-cyan-500 text-white rounded-md w-full mb-2">Info</button></a>
                                    <form action="{{ route('admin.data_dospem.delete_dospem', ['id' => $user->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
